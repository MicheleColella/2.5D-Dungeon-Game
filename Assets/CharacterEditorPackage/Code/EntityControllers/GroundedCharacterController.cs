...
public class GroundedCharacterController : CharacterControllerBase
{
    ...
    //[SerializeField, Range(0.01f, 4f)] float animSpeedControl = 1f;
...
//Jump event (for other scripts to use when the jump is triggered)
    ...
    public Vector2 Actual_velocity;
    ...
    protected override void DefaultUpdateMovement()
    {
        ...
        //Debug.Log(currentVel); // Velocità attuale
    ...
    public float GetInputForce()
    {
        ...
        {
            modifier = 2000f;
            //Player_sprites.SetFloat("runspeed", animSpeedControl);
        }
        else
        {
            //Player_sprites.SetFloat("runspeed", 1);
            modifier = m_WalkForce;
        }
    ...
    public Vector2 GetGravity()
    {
        ...
        //Debug.Log(actual_gravity);

        Vector2 fGravity = Vector2.down * actual_gravity;
        if (m_ApplyGravityIntoGroundNormal)
        {
            fGravity = -m_ControlledCollider.GetGroundedInfo().GetNormal() * actual_gravity;
        }
                ...
                else
                {
                    if (m_ControlledCollider.GetVelocity().y > 0)
                    {
                        ...
                        else
                        {
                            if (Mathf.Abs(m_ControlledCollider.GetVelocity().x) < 0.0001f)
                            {
                                ...
                                //Debug.Log(edge_touch);
                                //Debug.Log(wall_touch);
                                //Debug.Log(capsule_height);
                                if (capsule_height == 2.1f && isGrounded == false && edge_touch == false && wall_touch == false && dangling == false)
                                {
                                    return "CharacterSprite_Attack_melee_Jump";
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                if (capsule_height == 2.1f && isGrounded == false && edge_touch == false && wall_touch == false)
                                {
                                    return "CharacterSprite_Attack_melee_Jump";
                                }
                                else
                    ...
                    else
                    {
                        if (Mathf.Abs(m_ControlledCollider.GetVelocity().x) < 0.0001f)
                        {
                            ...
                            if (right == 1 && capsule_height == 2.1f && isGrounded == false && edge_touch == false && wall_touch == false)
                            {
                                return "CharacterSprite_Attack_melee_Jump";
                            }
                            else
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        // Apply the push
        body.velocity = pushDir * 100.0f;
    }

    private void Update()
    {
        Actual_velocity = m_ControlledCollider.GetVelocity();
    }
}
